...
Copy `ops/cloudflared-config.example.yml` to `ops/cloudflared-config.yml` (gitignored)
and fill in your tunnel. On my machine a shared controller runs it:
...
Its own tunnel (`spice`), not an ingress rule on the shared
...
address (`tailscale ip -4`). That is CGNAT space — routable inside the tailnet and
...
2. Copy `ops/Caddyfile.example` to `ops/Caddyfile` (gitignored), fill in your hostname and
   tailnet IP, then verify: `C:\Development\_ops\caddy\caddy.exe validate --config ops\Caddyfile`
   (it currently fails only on the empty token, which is the expected state)
3. Replace the `spice` CNAME with an `A` record → your tailnet IP, **grey cloud**.
...
a device off the tailnet, not a bug. *the tailnet `bind` fails at startup* means
