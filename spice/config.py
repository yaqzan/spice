...
def _load_local_env(path: Path) -> None:
    """Fill os.environ from the gitignored `.env` at the repo root.

    Machine-specific settings (your domain, your Obsidian path) live there so the
    tracked code carries no one's paths. A file rather than exported variables so
    a shell and the service wrapper see the same values. A variable already set
    in the real environment wins. See `.env.example`.
    """
    try:
...
PUBLIC_ORIGIN = os.environ.get('SPICE_PUBLIC_ORIGIN', '')
...
# The rating reminder: one checklist line per recipe in a Markdown file (the
# owner's is in an Obsidian vault). Off unless SPICE_TODO_FILE is set, normally
# in `.env` -- a file both a shell and the service wrapper read, so it cannot
# behave differently between them. If the folder is not there, spice/vault.py
# stays silent -- see .claude/docs/vault.md.
_todo = os.environ.get('SPICE_TODO_FILE', '').strip()
TODO_FILE = Path(_todo) if _todo else None
...
APP_TITLE = 'Spice'
APP_REFERER = PUBLIC_ORIGIN or 'https://github.com/yaqzan/spice'
...
TAILSCALE_EXE = os.environ.get('SPICE_TAILSCALE') or shutil.which('tailscale') or (
