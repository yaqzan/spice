...
def test_a_forwarded_header_cannot_forge_a_tailnet_address(public):
    ...
    for header in ('X-Forwarded-For', 'X-Real-IP', 'X-Client-IP',
        ...
        response = public.get('/api/settings', headers={header: '100.101.102.103'},
...
def test_the_tailnet_check_can_be_switched_off(public, monkeypatch):
    ...
    assert public.get('/api/settings',
                      environ_base={'REMOTE_ADDR': '100.101.102.103'}).status_code == 401
...
# ── local config: the gitignored .env ────────────────────────────────────────

def test_local_env_fills_blanks_but_never_overrides_the_real_environment(tmp_path, monkeypatch):
    from spice import config
    env = tmp_path / '.env'
    env.write_text('# comment\nSPICE_T_A=from-file\nSPICE_T_B="quoted"\nnot a setting\n'
                   'SPICE_T_C=from-file\n', encoding='utf-8')
    monkeypatch.delenv('SPICE_T_A', raising=False)
    monkeypatch.delenv('SPICE_T_B', raising=False)
    monkeypatch.setenv('SPICE_T_C', 'real')
    config._load_local_env(env)
    import os
    assert os.environ['SPICE_T_A'] == 'from-file'
    assert os.environ['SPICE_T_B'] == 'quoted'
    assert os.environ['SPICE_T_C'] == 'real'
    for key in ('SPICE_T_A', 'SPICE_T_B'):
        monkeypatch.delenv(key)


def test_a_missing_env_file_is_not_an_error(tmp_path):
    from spice import config
    config._load_local_env(tmp_path / 'nope.env')


def test_the_rating_reminder_is_off_without_a_configured_file(monkeypatch):
    from spice import vault
